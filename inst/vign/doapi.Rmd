...
There is a `print.droplet()` method that is used to print a brief summary 
of each droplet. 

In addition, you can get a more detailed summary of a droplet by using 
`summary()`. For example:
...
List actions on a droplet, newer ones at the top. Here's list actions, then 
rename and list actions again.
...
Making a snapshot of a droplet can be done with `droplet_snapshot()`. This action 
requires that you turn off the droplet first, then take the snapshot. First, create a droplet
